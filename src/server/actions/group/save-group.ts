...
export const saveGroup = async (data: SnobGroup): Promise<ActionResponse> => {
  try {
    ...
    const updatedGroupId = await updateOrCreateGroup(
      validatedData.data as SnobGroup,
      snob,
    );
    ...
    await db
      ...
      .where(
        and(
          eq(snobGroupAttributesTable.groupId, updatedGroupId),
          notInArray(snobGroupAttributesTable.id, attributesIds),
        ),
      );
...
const updateOrCreateAttribute = async (
  attribute: SnobGroupAttribute,
  groupId: string,
) => {
  const rows = await db
    .select()
