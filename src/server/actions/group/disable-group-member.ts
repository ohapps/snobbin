...
export const disableGroupMember = async (
  groupId: string,
  snobId: string,
  ...
  try {
    ...
    const results = await db
      ...
      .set({ role: SnobGroupRole.DISABLED })
      .where(
        and(
    ...
    if (results.length === 0) {
      throw new Error("No changes made, member not found or already disabled");
    }
    return { success: true };
  } catch (error) {
    return logAndReturnError("failed to disable group member", error);
  }
};
